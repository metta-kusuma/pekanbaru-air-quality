...
# Ambil API Keys dari GitHub Secrets / Environment Variables
...
def fetch_and_log():
    # Penanganan Zona Waktu WIB yang Akurat
    wib = pytz.timezone('Asia/Jakarta')
    now_wib_dt = datetime.now(wib)
    timestamp_wib = now_wib_dt.strftime("%Y-%m-%d %H:%M:%S")
    ...
    if AQICN_API_KEY:
        aqicn_url = f"https://api.waqi.info/feed/geo:{LAT};{LON}/?token={AQICN_API_KEY}"
        try:
    ...
    # ==========================================================
    # 3. PREDIKSI CUACA (Dieksekusi HANYA Setiap 3 Jam Sekali)
    # ==========================================================
    # Mengecek apakah jam WIB saat ini adalah kelipatan 3 (00, 03, 06, 09, 12, 15, 18, 21)
    current_hour_wib = now_wib_dt.hour

    if OPENWEATHER_API_KEY and (current_hour_wib % 3 == 0):
        ...
        try:
            ...
            if res_f.status_code == 200:
                ...
                for item in forecast_list:
                    ...
                    dt_wib = (dt_utc + pd.Timedelta(hours=7)).strftime("%Y-%m-%d %H:%M:%S")

                    main_f = item.get("main", {})
                    w_f = item.get("weather", [{}])[0]

                    forecast_rows.append({
                        "Fetched_Timestamp": timestamp_wib,
                        "Forecast_Target_WIB": dt_wib,
                        "City": CITY,
                ...
                print(f"[{timestamp_wib}] Sukses mencatat dataset Perkiraan Cuaca (Interval 3 Jam).")
        except Exception as e:
            print("Error OpenWeather Forecast:", e)
    else:
        print(f"[{timestamp_wib}] Skip Forecast API (Jam {current_hour_wib:02d}:00 WIB bukan kelipatan 3).")
