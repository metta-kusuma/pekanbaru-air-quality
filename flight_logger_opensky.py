...
def get_airline_name(callsign):
    ...
    # Deteksi Pesawat Privat / Charter Registrasi Khusus
    ...
    airlines = {
        ...
        'SJV': 'Sriwijaya Air / Super Air Jet',
        'SJY': 'Sriwijaya Air',
        'NAM': 'NAM Air',
        ...
        'SPRN': 'Super Air Jet (Alt)',
        'KCN': 'K-Mile Air / Kencana Air',
        'JDE': 'IndiGo / Eastindo Charter',
        'RGM': 'Rimbun Air',
        'CTW': 'Citilink (Cargo/Charter)',
        'MKT': 'Mokulele / Smart Aviation',
        'TAG': 'Express Transportasi Antarbenua',
        'OEY': 'Other Charter / Executive',
        'FHS': 'Flight Inspection / Helicopter',
        ...
        'CDG': 'Shandong Airlines / China Eastern',
        'CES': 'China Eastern Airlines',
        'HVN': 'Vietnam Airlines',
        'KAL': 'Korean Air',
        'KMI': 'K-Mile Air',
        'MXD': 'Batik Air Malaysia (Malindo)',
        'ETD': 'Etihad Airways',
        'SVA': 'Saudia (Saudi Arabian)',
        'THY': 'Turkish Airlines',
        'QTR': 'Qatar Airways',
        'JST': 'Jetstar Airways',
        'MYU': 'My Indo Airlines',
        'QQE': 'Qatar Executive / Charter',
        
        # --- MASKAPAI GLOBAL LAINNYA ---
        'UAE': 'Emirates', 'SUD': 'Saudi Arabian Airlines', 'SWR': 'Swiss International Air Lines',
        'GFA': 'Gulf Air', 'OMA': 'Oman Air', 'RJA': 'Royal Jordanian', 'KAC': 'Kuwait Airways',
        'SIA': 'Singapore Airlines', 'SLK': 'SilkAir', 'MAS': 'Malaysia Airlines',
        'AXM': 'AirAsia (Malaysia)', 'MYX': 'MYAirline', 'THA': 'Thai Airways International',
        'AIQ': 'Thai AirAsia', 'TLM': 'Thai Lion Air', 'CPA': 'Cathay Pacific',
        'HDA': 'Cathay Dragon / HK Express', 'HKP': 'Hong Kong Express Airways',
        ...
        'AAL': 'American Airlines', 'DAL': 'Delta Air Lines', 'FDX': 'FedEx Express',
        'UPS': 'UPS Airlines', 'GTI': 'Atlas Air', 'BOX': 'Aerologic (DHL Cargo)',
        'PAC': 'Polar Air Cargo', 'SQC': 'Singapore Airlines Cargo'
    }
    
    return airlines.get(prefix, 'Other Airline')

# -------------------------------------------------------------------
# 1. OPENSKY DATA FETCHING (ASLI MILIK ANDA)
# -------------------------------------------------------------------
def fetch_opensky_flights(now_wib):
    ...
    params = {
        'lamin': -6.85, 'lomin': 105.80, 'lamax': -5.50, 'lomax': 107.50
    }
...
# -------------------------------------------------------------------
# 2. FLIGHTRADAR24 DATA FETCHING (TAMBAHAN UNTUK CSV KE-2)
# -------------------------------------------------------------------
        ...
        if isinstance(flights, list) and len(flights) > 0:
            for f in flights:
                ...
                flight_data.append({
                    ...
                    'Airline_ICAO': f.airline_icao or 'N/A',
                    'Aircraft_Model': f.aircraft_code or 'N/A',
                    'Registration_Number': f.registration or 'N/A',
                    'Origin_IATA': f.origin_airport_iata or 'N/A',
                    'Destination_IATA': f.destination_airport_iata or 'N/A',
            ...
            flight_data.append({
                'Timestamp_WIB': now_wib, 'Callsign': 'NONE', 'Flight_Number': 'NONE',
                'Airline_ICAO': 'N/A', 'Aircraft_Model': 'N/A', 'Registration_Number': 'N/A',
                'Origin_IATA': 'N/A', 'Destination_IATA': 'N/A',
                'Latitude': -6.1256, 'Longitude': 106.6558, 'Heading_Deg': 0,
                'Altitude_Meters': 0, 'Altitude_Feet': 0, 'Ground_Speed_KMH': 0,
                'Ground_Speed_Knots': 0, 'Vertical_Speed_MS': 0, 'Vertical_Speed_FPM': 0,
                'Flight_Phase': 'No Flights', 'Is_Ground': 0, 'Source': 'FlightRadar24'
            })

        df_new = pd.DataFrame(flight_data)
        df_new.to_csv(csv_file, mode='a' if os.path.exists(csv_file) else 'w', header=not os.path.exists(csv_file), index=False)
        print(f"[{now_wib}] FlightRadar24 Success: Berhasil mencatat {len(flights) if flights else 0} penerbangan.")
...
if __name__ == "__main__":
    ...
    # Jalankan OpenSky (menghasilkan jakarta_flights_log_opensky.csv)
    fetch_opensky_flights(now_wib)
    
    # Jalankan FlightRadar24 (menghasilkan jakarta_flights_log_fr24.csv)
